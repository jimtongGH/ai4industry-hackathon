...
// Handle a package order received from another agent (e.g. the packaging
// workshop), then acknowledge it back to the sender (Step 4 coordination).
+!orderPackages(Value)[source(Sender)] :
    true
    <-
    !orderPackages(cupProvider,Value);
    .println("processed package order and sending message to ",Sender);
...
// Organisational stage hook: as a `supplier` in the production_line group, the
// provider is obliged to the supply_material goal. It serves on demand via
// order/orderPackages messages, so this goal simply reports readiness.
+!supply_material : true
    <-
    .print("[org] supplier ready to deliver material on request.");
  .
