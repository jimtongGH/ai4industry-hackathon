 ...
 *
 * This is a local override of the upstream jason-interpreter class. The upstream
 * append(String, String) mutates Swing components (JFrame/JTextArea) directly from
 * whatever thread calls it (typically the "main" thread), instead of dispatching to
 * the Event Dispatch Thread. That races with Swing's own layout/repaint thread and can
 * deadlock (main thread holds the text document's write lock while trying to acquire
 * the AWT tree lock for a caret update, while the EDT holds the tree lock and waits on
 * the document's read lock). Routing all component mutations through
 * SwingUtilities.invokeLater fixes this: everything happens on the EDT, in the order
 * append() was called, with no cross-thread Swing access.
 */
public class MASConsoleGUI {
    ...
    public void append(final String agName, final String s) {
        if (inPause) {
            waitNotPause();
        }
        // All Swing component access must happen on the Event Dispatch Thread.
        // invokeLater preserves call order (the EDT processes queued Runnables
        // FIFO), so console output still appears in the order append() was
        // called, it just happens asynchronously instead of racing the EDT.
